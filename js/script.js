...
(function(){
  ...
  var cooldown=false,exitedDown=false;
  ...
  function doRelease(dir){
    ...
    if(dir==='down'){
      ...
      setTimeout(function(){
        ...
        setTimeout(function(){if(veil)veil.classList.remove('tc-eb-on');},820);
      },180);
    } else {
  ...
  function onW(e){
    ...
    wt=setTimeout(function(){
      if(Math.abs(wAcc)>30)go(wAcc>0?1:-1);
      wAcc=0;
    },50);
  }
  ...
  window.addEventListener('scroll',function(){
    ...
    if(cooldown)return;
    if(exitedDown){
      if(sec.getBoundingClientRect().top>window.innerHeight*0.5){exitedDown=false;}
      else return;
    }
    if(!goingDown)return;
...
function addCTA(){rmTyp();const a=document.createElement('a');a.className='chat-cta';a.href='https://wa.me/5527981256362?text=Olá! Quero posicionar minha empresa no topo do Google';a.target='_blank';a.rel='noopener noreferrer';
...
document.querySelectorAll('.cta-btn').forEach(b=>{let lt=0,r=null;b.addEventListener('mouseenter',()=>{r=b.getBoundingClientRect()});b.addEventListener('mousemove',e=>{const n=Date.now();if(n-lt<32||!r)return;lt=n;b.style.boxShadow=(e.clientX-r.left-r.width/2)+'px '+(e.clientY-r.top-r.height/2)+'px 40px rgba(123,60,255,.2)'},{passive:true});b.addEventListener('mouseleave',()=>{r=null;b.style.boxShadow=''})});
...
(function(){
  ...
  var N=window.innerWidth<768?38:60;
  ...
  if(document.readyState==='complete'){resize();raf=requestAnimationFrame(draw);}
  else{window.addEventListener('load',function(){resize();raf=requestAnimationFrame(draw);});}
})();
...
(function(){
  ...
  // Muted badge click → unmute + reinicia do começo com som
